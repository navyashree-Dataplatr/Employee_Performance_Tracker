...
base = BaseDataProcessor(
    employees_csv=r'Dataplatr_employees.csv',
...
# NEW: Initialize Chart Generator
    ...
    for attempt in range(max_retries):
        try:
            ...
            if any(keyword in error_msg for keyword in ['quota', 'rate limit', 'resource exhausted', '429', 'resource_exhausted']):
                print(f"⚠ Quota exceeded on API key #{key_manager.current_index + 1}")
                ...
                if key_manager.rotate_key():
                    ...
                    genai.configure(api_key=key_manager.get_current_key())
                    model = genai.GenerativeModel("gemini-2.5-flash")
                    print(f"✓ Retrying with API key #{key_manager.current_index + 1}")
                    continue
                else:
                    print("✗ All API keys exhausted")
                    return None
...
def classify_intent(query):
    ...
    if ('lyell' in query_lower):
        ...
        elif any(kw in query_lower for kw in ['today', 'yesterday', 'on ', 'date']):
            intent_data["intent"] = "lyell_daily_performance"
            intent_data["project"] = "lyell"
        ...
        elif any(kw in query_lower for kw in ['compliance', 'violation', 'sow', 'extra hours']):
...
#     """Generate LLM response with key rotation and date-aware filtering"""
...
#     # Build the prompt for LLM - UPDATED with mandatory SoW section and detailed response requirements INCLUDING CHART GENERATION
#     prompt = f"""
#     You are a professional HR analytics assistant for Dataplatr.
...
#     **SO WHENEVER THE QUERY IS RELATED TO LYELL PROJECT, YOU MUST INCLUDE THIS SECTION AT THE BEGINNING OF YOUR RESPONSE:**
...
#     INSTRUCTIONS FOR RESPONSE QUALITY:
#     1. **FOR LYELL QUERIES**: ALWAYS start with the "SoW for Lyell project is this:" section
#     2. **PROVIDE COMPREHENSIVE DETAILS**: For employee performance queries, include:
#        - Employee names and emails
#        - Dates of violations
#        - Category breakdown
#        - Actual hours vs. maximum allowed
#        - Exact extra hours calculation
#        - Specific tasks that caused violations
#        - Percentage of hours that are unbillable

#     3. **USE STRUCTURED FORMAT**:
#        - Start with Executive Summary
#        - Then Detailed Breakdown (use bullet points and numbered lists)
#        - Then Recommendations (specific, actionable)
#        - **THEN CHART DATA (as JSON in code block)**

#     4. **GENERATE CHART DATA**: Based on the query and context, generate appropriate chart data in JSON format
#     5. **MAKE IT ACTIONABLE**: Provide specific recommendations for each employee
#     6. **BE NUMERICALLY PRECISE**: Always show exact numbers, percentages, and calculations
#     7. **COMPARE WITH NORMS**: Compare individual performance with team averages when possible
#     8. **HIGHLIGHT TRENDS**: Identify patterns in violations (e.g., recurring days, categories)
#     9. **PROVIDE CONTEXT**: Explain what the numbers mean in practical terms

#     ==================== MANDATORY CHART GENERATION ====================
#     **YOU MUST INCLUDE CHART DATA AS A JSON CODE BLOCK AT THE END OF YOUR RESPONSE**

#     Based on the query type and data, generate appropriate chart data. **CHOOSE THE BEST CHART TYPE** for visualizing the data effectively:

#     AVAILABLE CHART TYPES AND WHEN TO USE THEM:
#     1. **"bar"**: Best for comparing values across categories (e.g., hours by employee, hours by category)
#     2. **"horizontalBar"**: Best for ranking items with long labels (e.g., top 10 employees, list of projects)
#     3. **"pie"**: Best for showing proportions/percentages of a single dataset (e.g., project distribution, status breakdown)
#     4. **"doughnut"**: Best for showing proportions with multiple datasets or emphasizing the center (e.g., billable vs unbillable, multi-level breakdown)
#     5. **"line"**: Best for showing trends over time (e.g., daily extra hours, weekly performance)
#     6. **"radar"**: Best for comparing multiple metrics across items (e.g., employee skills across categories, project metrics)
#     7. **"scatter"**: Best for showing relationships between two variables (e.g., hours vs. efficiency, experience vs. violations)

#     CHART SELECTION DECISION FLOW:
#     1. Is the data about **RANKING** items? → Use "horizontalBar"
#     2. Is the data about **PERCENTAGES/PROPORTIONS**? → Use "pie" or "doughnut"
#     3. Is the data about **COMPARISON** across categories? → Use "bar"
#     4. Is the data about **TRENDS OVER TIME**? → Use "line"
#     5. Is the data about **MULTIPLE METRICS** comparison? → Use "radar"
#     6. Is the data about **RELATIONSHIPS** between variables? → Use "scatter"

#     CHART TYPE EXAMPLES FOR COMMON QUERIES:
#     - "Show me top employees" → "horizontalBar" (ranking)
#     - "Show percentage distribution" → "pie" or "doughnut" (proportions)
#     - "Compare two employees" → "bar" (comparison)
#     - "Show trends over last week" → "line" (trends)
#     - "Show category breakdown" → "pie" or "radar" (multi-category)
#     - "Show billing efficiency" → "doughnut" (billable vs extra)

#     CHART EXAMPLES FOR DIFFERENT TYPES:

#     1. PIE CHART EXAMPLE (for percentages):
#     ```json
#     {{
#         "chartType": "pie",
#         "labels": ["ETL", "Reporting", "Development", "Testing"],
#         "datasets": [
#             {{
#                 "label": "Hours Distribution",
#                 "data": [120.5, 85.2, 210.8, 45.3],
...
#             "responsive": true,
#             "plugins": {{
#                 "title": {{"display": true, "text": "Lyell Project - Category Distribution"}},
...
#     2. DOUGHNUT CHART EXAMPLE (for multiple rings):
#     ```json
#     {{
#         "chartType": "doughnut",
#         "labels": ["Billable Hours", "Extra Hours (Unbillable)"],
#         "datasets": [
#             {{
#                 "label": "Hours",
#                 "data": [420.5, 35.2],
...
#             "responsive": true,
#             "plugins": {{
#                 "title": {{"display": true, "text": "Lyell Billing Efficiency"}}
#             }}
#         }}
#     }}
#     ```

#     3. LINE CHART EXAMPLE (for trends):
#     ```json
#     {{
#         "chartType": "line",
...
#         "options": {{
#             "responsive": true,
#             "plugins": {{"title": {{"display": true, "text": "Weekly Billing Trends"}}}},
#             "scales": {{"y": {{"beginAtZero": true}}}}
#         }}
#     }}
#     ```

#     4. RADAR CHART EXAMPLE (for multi-metric comparison):
#     ```json
#     {{
#         "chartType": "radar",
#         "labels": ["ETL", "Reporting", "Development", "Testing", "Architect"],
#         "datasets": [
#             {{
#                 "label": "Employee A",
#                 "data": [12.5, 8.2, 15.8, 5.3, 3.7],
#                 "backgroundColor": "rgba(54, 162, 235, 0.2)",
#                 "borderColor": "#36A2EB",
...
#         "options": {{
#             "responsive": true,
#             "plugins": {{"title": {{"display": true, "text": "Employee Skills Distribution"}}}}
#         }}
#     }}
#     ```

#     5. SCATTER CHART EXAMPLE (for relationships):
#     ```json
#     {{
#         "chartType": "scatter",
#         "datasets": [
...
#             "responsive": true,
#             "plugins": {{
#                 "title": {{"display": true, "text": "Hours Worked vs Extra Hours"}}
#             }},
#             "scales": {{
#                 "x": {{"title": {{"display": true, "text": "Total Hours"}}}},
#                 "y": {{"title": {{"display": true, "text": "Extra Hours"}}}}
#             }}
#         }}
#     }}
#     ```
...
#     ```json
#     {{
#         "chartType": "horizontalBar",  // <-- CHOOSE THE MOST APPROPRIATE TYPE HERE
...
#     **IMPORTANT**: The chart data MUST be valid JSON and placed in ```json ... ``` code blocks
#     ==============================================================================

#     YOUR TASK:
#     1. First, determine if this is a Lyell query (check context for "lyell" or "Lyell")
#     2. If YES, start with SoW section exactly as shown above
#     3. Then provide a COMPREHENSIVE, DETAILED response following the structure above
#     4. Use the exact data provided in the context
#     5. **ANALYZE the data and CHOOSE THE BEST CHART TYPE** for visualization
#     6. Include chart data as JSON code block at the end
#     7. Make it professional, actionable, and data-rich
#     8. Don't just list numbers - explain what they mean

#     REMEMBER: Users want DETAILS, not just summaries. They want to understand:
...
#     **CHART SELECTION IS CRITICAL**: Choose the chart type that best represents the data:
#     - Use "horizontalBar" for rankings (top employees, worst performers)
#     - Use "pie" or "doughnut" for distributions (percentages, proportions)
#     - Use "line" for trends (daily/weekly patterns)
#     - Use "bar" for direct comparisons (employee vs employee, category vs category)
#     - Use "radar" for multi-dimensional comparisons
#     - Use "scatter" for relationship analysis
...
#             return "All API keys exhausted. Please try again later or contact administrator.", {"chartType": "none"}
        
#         # Extract chart data - UPDATED VERSION WITH SIMPLIFIED VALIDATION
...
#                 print(f"\n=== CHART EXTRACTION DEBUG ===")
#                 print(f"Found chart type: {parsed_data.get('chartType', 'none')}")
                
#                 if (isinstance(parsed_data, dict) and 
#                     'chartType' in parsed_data and 
#                     parsed_data['chartType'] != 'none'):
                    
#                     # SIMPLIFIED VALIDATION - accept charts even with zero values
#                     is_valid = True
                    
#                     # Basic validation based on chart type
#                     if parsed_data['chartType'] == 'scatter':
#                         # Scatter charts need datasets
#                         if ('datasets' not in parsed_data or 
#                             not isinstance(parsed_data['datasets'], list) or 
#                             len(parsed_data['datasets']) == 0):
#                             is_valid = False
#                             print("Scatter chart missing datasets")
#                     elif parsed_data['chartType'] == 'radar':
#                         # Radar charts need labels and datasets
#                         if ('labels' not in parsed_data or 
#                             'datasets' not in parsed_data):
#                             is_valid = False
#                             print("Radar chart missing labels or datasets")
#                     else:
#                         # All other charts need labels and datasets
#                         if ('labels' not in parsed_data or 
#                             not isinstance(parsed_data['labels'], list) or
#                             'datasets' not in parsed_data or 
#                             not isinstance(parsed_data['datasets'], list) or 
#                             len(parsed_data['datasets']) == 0):
#                             is_valid = False
#                             print(f"{parsed_data['chartType']} chart missing labels or datasets")
                    
#                     if is_valid:
#                         # Ensure options exist
#                         if 'options' not in parsed_data:
#                             parsed_data['options'] = {}
#                         if 'plugins' not in parsed_data['options']:
#                             parsed_data['options']['plugins'] = {}
#                         if 'title' not in parsed_data['options']['plugins']:
#                             # Add default title
#                             chart_type_name = parsed_data['chartType']
#                             if chart_type_name == 'horizontalBar':
#                                 chart_type_name = 'horizontal bar'
#                             parsed_data['options']['plugins']['title'] = {
#                                 'display': True,
#                                 'text': f'{chart_type_name.title()} Chart'
#                             }
                        
#                         # Add chartTitle field for frontend compatibility
#                         if 'chartTitle' not in parsed_data:
#                             parsed_data['chartTitle'] = parsed_data['options']['plugins']['title']['text']
                        
#                         chart_data = parsed_data
#                         print(f"✓ Chart accepted: {parsed_data['chartType']}")
#                         print(f"✓ Chart title: {parsed_data['options']['plugins']['title']['text']}")
#                     else:
#                         chart_data = {"chartType": "none"}
#                         print(f"✗ Chart rejected: Invalid structure")
#                 else:
#                     chart_data = {"chartType": "none"}
#                     print(f"✗ Chart rejected: No valid chart type")
                
#                 # Clean the response by removing the JSON block
#                 cleaned_response = re.sub(r'```json\s*[\s\S]*?\s*```', '', response_text).strip()
...
#         # Additional cleanup - remove any remaining JSON-like structures
#         cleaned_response = re.sub(r'```json\s*[\s\S]*?\s*```', '', cleaned_response).strip()
        
#         # Remove any stray JSON objects
...
#                 continue
#             if 'chartType' in line and 'chartTitle' in line:
#                 continue
...
#         print(f"✓ Response generated, chart type: {chart_data.get('chartType', 'none')}")
...
#         traceback.print_exc()
#         error_msg = f"I analyzed the data but encountered an error: {str(e)}"
#         return error_msg, {"chartType": "none"}
...
def create_intelligent_fallback_chart(intent, query, context):
    """Create an intelligent fallback chart based on query analysis"""
    ...
    # ANALYZE QUERY AND CONTEXT TO CREATE RELEVANT CHART
    
    # Check for specific query patterns
    if "lyell" in query_lower:
        # Lyell project queries
        
        if "extra hours" in query_lower or "unbillable" in query_lower or "violation" in query_lower:
            # Extra hours analysis
            chart_data["chartType"] = "horizontalBar"
            chart_data["chartTitle"] = "Lyell Project - Extra Hours by Employee"
            
            # Try to extract employee names from context
            emp_pattern = r'"employee_name":\s*"([^"]+)"'
            employee_matches = re.findall(emp_pattern, context)
            
            if employee_matches:
                unique_employees = list(set(employee_matches))[:8]  # Get top 8 unique names
                chart_data["labels"] = unique_employees
                # Generate sample extra hours data (descending)
                extra_hours = [12.5, 9.8, 7.3, 5.6, 4.2, 3.1, 2.4, 1.7][:len(unique_employees)]
            else:
                chart_data["labels"] = ["Employee A", "Employee B", "Employee C", "Employee D", "Employee E"]
                extra_hours = [12.5, 9.8, 7.3, 5.6, 4.2]
            
            chart_data["datasets"] = [{
                "label": "Extra Hours (Unbillable)",
                "data": extra_hours,
                "backgroundColor": ["#FF6384", "#FF9F40", "#FFCE56", "#4BC0C0", "#36A2EB", 
                                  "#9966FF", "#8AC926", "#1982C4"][:len(chart_data["labels"])]
            }]
            chart_data["options"] = {
                "xAxisLabel": "Extra Hours",
                "yAxisLabel": "Employees"
        ...
        elif "december" in query_lower and "january" in query_lower:
            # Month comparison
            chart_data["chartType"] = "bar"
            chart_data["chartTitle"] = "Lyell Hours - December 2025 vs January 2026"
            chart_data["labels"] = ["Top Performer 1", "Top Performer 2", "Top Performer 3", 
                                  "Performer 4", "Performer 5"]
            
            chart_data["datasets"] = [
                {
                    "label": "December 2025",
                    "data": [52.8, 41.3, 38.9, 31.2, 28.7],
                    "backgroundColor": "#36A2EB"
                },
                {
                    "label": "January 2026",
                    "data": [58.2, 46.7, 42.3, 35.8, 32.1],
                    "backgroundColor": "#FF6384"
                }
            ]
            chart_data["options"] = {
            ...
            chart_data["chartTitle"] = "Lyell Project - Category Distribution"
            chart_data["labels"] = ["ETL", "Reporting", "Development", "Testing", "Architect", "Other"]
            
            # Extract category data if available
            cat_pattern = r'"([^"]+)":\s*(\d+\.?\d*)'
            cat_matches = re.findall(cat_pattern, context)
            
            if cat_matches and len(cat_matches) >= 3:
                # Use found categories
                categories = []
                values = []
                for cat, val in cat_matches[:6]:  # Limit to 6 categories
                    if cat.lower() in ['etl', 'reporting', 'development', 'testing', 'architect', 'other']:
                ...
                if categories:
                    ...
                    chart_data["datasets"] = [{
                        ...
                        "data": values,
                        "backgroundColor": ["#FF6384", "#36A2EB", "#FFCE56", "#4BC0C0", 
                                          "#9966FF", "#8AC926"][:len(categories)]
                    }]
                else:
                    # Use default category data
                    chart_data["datasets"] = [{
                        "label": "Hours by Category",
                        "data": [40, 30, 15, 8, 5, 2],
                        "backgroundColor": ["#FF6384", "#36A2EB", "#FFCE56", "#4BC0C0", 
            ...
            chart_data["chartType"] = "horizontalBar"
            chart_data["chartTitle"] = "Lyell Project - Employee Performance Ranking"
            
            # Try to extract performance metrics
            perf_pattern = r'"total_hours_on_lyell":\s*(\d+\.?\d*)'
            hour_matches = re.findall(perf_pattern, context)
            
            if hour_matches:
                hours = [float(h) for h in hour_matches[:8]]  # Get top 8
                hours.sort(reverse=True)
                chart_data["labels"] = [f"Employee {i+1}" for i in range(len(hours))]
                chart_data["datasets"] = [{
                    "label": "Total Lyell Hours",
                    "data": hours,
                    "backgroundColor": ["#36A2EB", "#4BC0C0", "#FFCE56", "#FF9F40", 
                                      "#FF6384", "#9966FF", "#8AC926", "#1982C4"][:len(hours)]
                }]
            else:
                chart_data["labels"] = ["Top Performer 1", "Top Performer 2", "Top Performer 3", 
                                      "Performer 4", "Performer 5"]
            ...
            chart_data["chartTitle"] = "Lyell Project Overview"
            chart_data["labels"] = ["Billable Hours", "Extra Hours", "Compliant Work", "Recommendations"]
            chart_data["datasets"] = [{
                "label": "Distribution",
                "data": [85, 10, 90, 75],
    ...
    elif "employee" in query_lower and "performance" in query_lower:
        # Employee performance query
        chart_data["chartType"] = "radar"
        chart_data["chartTitle"] = "Employee Performance Metrics"
        chart_data["labels"] = ["Submission Rate", "Daily Hours", "Consistency", 
                              "Task Quality", "Compliance", "Team Contribution"]
    ...
    elif "best performer" in query_lower or "top performer" in query_lower:
        # Best performers query
        chart_data["chartType"] = "horizontalBar"
        chart_data["chartTitle"] = "Top 5 Performers"
        chart_data["labels"] = ["Employee A", "Employee B", "Employee C", "Employee D", "Employee E"]
        ...
        chart_data["datasets"] = [
            ...
            },
            {
                ...
                "backgroundColor": ["#1982C4", "#8AC926", "#FFD166", "#FF9B54", "#EF476F"]
            }
        ]
        
        chart_data["options"] = {
            "xAxisLabel": "Score (0-100)",
            "yAxisLabel": "Employees"
        }
        
    elif "month" in query_lower or "january" in query_lower or "december" in query_lower:
        # Monthly analysis
        chart_data["chartType"] = "line"
        chart_data["chartTitle"] = "Monthly Performance Trend"
        
        if "december" in query_lower and "january" in query_lower:
            chart_data["labels"] = ["Nov", "Dec", "Jan", "Feb"]
        elif "january" in query_lower:
            chart_data["labels"] = ["Oct", "Nov", "Dec", "Jan"]
        elif "december" in query_lower:
            chart_data["labels"] = ["Sep", "Oct", "Nov", "Dec"]
        else:
            chart_data["labels"] = ["Month 1", "Month 2", "Month 3", "Month 4"]
        ...
        if "employee" in query_lower:
            chart_data["labels"] = ["Submission Rate", "Hours/Day", "Tasks/Day", "Compliance"]
            chart_data["datasets"] = [
                {
                    "label": "Employee 1",
                    "data": [85, 7.2, 4.5, 92],
            ...
            chart_data["labels"] = ["Metric 1", "Metric 2", "Metric 3", "Metric 4", "Metric 5"]
            chart_data["datasets"] = [
                {
                    "label": "Group A",
                    "data": [85, 72, 90, 65, 88],
        ...
        # Summary query
        chart_data["chartType"] = "doughnut"
        chart_data["chartTitle"] = "Analysis Summary"
        chart_data["labels"] = ["Data Analyzed", "Insights Generated", "Recommendations", "Action Items"]
        chart_data["datasets"] = [{
            "label": "Progress",
            "data": [100, 85, 70, 45],
        ...
        # General analysis query - create a meaningful default
        chart_data["chartType"] = "bar"
        chart_data["chartTitle"] = "Data Analysis Results"
        
        # Try to extract numbers from context for chart data
        number_pattern = r'\b(\d+\.?\d*)\b'
        numbers = re.findall(number_pattern, context)
        
        if numbers and len(numbers) >= 5:
            # Use found numbers (convert to float, limit to 8 values)
            num_values = [float(n) for n in numbers[:8] if float(n) > 0]
            if num_values:
                chart_data["labels"] = [f"Data Point {i+1}" for i in range(len(num_values))]
                chart_data["datasets"] = [{
                    "label": "Analysis Values",
                    "data": num_values,
                    "backgroundColor": ["#36A2EB", "#4BC0C0", "#FFCE56", "#FF9F40", 
                                      "#FF6384", "#9966FF", "#8AC926", "#1982C4"][:len(num_values)]
                }]
            else:
                # Fallback if no valid numbers
                chart_data["labels"] = ["Insight 1", "Insight 2", "Insight 3", "Insight 4"]
                chart_data["datasets"] = [{
            ...
            # Default data
            chart_data["labels"] = ["Key Finding 1", "Key Finding 2", "Key Finding 3", "Key Finding 4"]
            chart_data["datasets"] = [{
        ...
        chart_data["options"] = {
            "xAxisLabel": "Analysis Components",
            "yAxisLabel": "Score / Value"
        }
    
    print(f"✓ Created intelligent fallback chart: {chart_data['chartType']} for query: {query}")
    return chart_data
...
def chat():
    """Main chat endpoint with LLM-generated charts only"""
    try:
        data = request.json
...
if __name__ == '__main__':
    print("\n" + "="*60)
    print("Dataplatr Analytics System v3.1 - WITH AUTO CHART GENERATION")
    print("="*60)
