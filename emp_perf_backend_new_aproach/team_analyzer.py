...
class TeamAnalyzer:
    ...
    def get_team_overview_metrics(self):
        ...
        for metrics in all_metrics:
            if 'project_distribution' in metrics and metrics['project_distribution']:
                for project, details in metrics['project_distribution'].items():
                    if project not in project_distribution:
                        project_distribution[project] = {
                            'total_employees': 0,
                            'total_hours': 0,
                            'employee_count': 0
                        }
                    project_distribution[project]['total_employees'] += 1
                    project_distribution[project]['total_hours'] += details['hours']
                    project_distribution[project]['employee_count'] += 1
        
        # Cross-project employees
        cross_project_employees = 0
        for metrics in all_metrics:
            if 'project_distribution' in metrics and len(metrics['project_distribution']) > 1:
                cross_project_employees += 1
        ...
        return {
            ...
            'cross_project_employees': cross_project_employees,
            'avg_project_hours': round(df['total_hours'].mean(), 2) if 'total_hours' in df.columns else 0
        }
    ...
    def get_project_focus_employees(self, project_name):
        """Get employees primarily working on a specific project"""
        focused_employees = []
        for primary_email in self.base.employee_all_emails.keys():
            metrics = self.individual.get_employee_detailed_metrics(primary_email)
            if (metrics and 'primary_project' in metrics and 
                metrics['primary_project'] and 
                project_name.lower() in str(metrics['primary_project']).lower()):
                focused_employees.append(metrics)
